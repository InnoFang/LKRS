...
namespace PSO {
    uint64_t encode(const uint64_t& pid, const uint64_t& sid, const uint64_t& oid, const uint64_t& so_mask) {
        uint64_t pso = (pid << (so_mask << 3) | (sid << (so_mask << 2) | oid;
        return pso;
    }

    uint64_t decode(const uint64_t& pso, const uint64_t& p_mask, const uint64_t& so_mask) const {
